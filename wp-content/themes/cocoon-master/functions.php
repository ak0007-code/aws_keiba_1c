...
// 予想ツールページ、携帯orPCで表示するHTMLファイルを変える
function includeFile() {
        $user_agent = $_SERVER['HTTP_USER_AGENT'];
        if (strpos($user_agent,'iPhone') === false && strpos($user_agent,'Android') === false && strpos($user_agent,'iPad') === false) {
                $file = '../../../execution.html';
        }else{
...
// 自動予想ページ、携帯orPCで表示するHTMLファイルを変える
function auto_predict_includeFile() {
        $user_agent = $_SERVER['HTTP_USER_AGENT'];
        if (strpos($user_agent,'iPhone') === false && strpos($user_agent,'Android') === false && strpos($user_agent,'iPad') === false) {
                $file = '../../../auto_predict.html';
        }else{
                $file = '../../../auto_predict_mobile.html';
        }
        return file_get_contents(dirname(__FILE__) . '/' . $file);
}
